  ...
  useEffect(() => {
    const initPayment = async () => {
      ...
      try {
        ...
        console.warn('Event payment init failed (use demo flow):', err?.message);
    ...
    initPayment();
  }, [event?.id, event?.title, selectedDays?.length, amountInCents]);
      ...
      <View style={styles.container}>
        ...
        <View style={[styles.centered, { padding: 24 }]}>
          ...
          <Text style={styles.demoNote}>
            For demo purposes, you can simulate a successful payment.
          </Text>
          <TouchableOpacity
            ...
            onPress={() =>
              navigation.replace(STACKS.EVENT_CONFIRMATION, {
                event,
                selectedDays,
                transactionId: 'GTS-DEMO-' + Date.now().toString(36).toUpperCase(),
                totalPaid: totalAmount,
              })
            }
          >
            <Text style={styles.demoButtonText}>Continue with Demo Payment</Text>
          </TouchableOpacity>
        </View>
      </View>
    );
